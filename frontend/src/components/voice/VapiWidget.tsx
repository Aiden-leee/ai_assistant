...
import { getMonthlyCallCount, createVoiceCall, type VoiceTranscriptMessage } from '@/lib/actions/voice/voice';
...
function VapiWidget() {
    ...
    const saveCallRecord = useCallback(async (status: 'COMPLETED' | 'FAILED' = 'COMPLETED') => {
        if (!user?.id || !callStartTime) return false;

        try {
            const endTime = new Date();
            const duration = Math.floor((endTime.getTime() - callStartTime.getTime()) / 1000); // 초 단위
    ...
    useEffect(() => {
        ...
        const handleCallEnd = async (data?: any) => {
            ...
            // 통화 기록 저장
            await saveCallRecord('COMPLETED');
        ...
        const handleError = async (error: Error) => {
            ...
            if (user?.id && callStartTime) {
                ...
                try {
                    ...
                    console.error('Error saving failed call record (connection failed):', err);
                }
            }
            setCallStartTime(null);
        }
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = (e: BeforeUnloadEvent) => {
            if (callActive && callStartTime && user?.id) {
        ...
        const handleVisibilityChange = async () => {
            if (document.hidden && callActive && callStartTime && user?.id) {
                // 페이지가 숨겨질 때 통화 기록 저장
                await saveCallRecord('COMPLETED');
            }
        };
        ...
        return () => {
            ...
            if (callActive && callStartTime && user?.id) {
                // 비동기 저장 시도 (실패할 수 있음)
                saveCallRecord('COMPLETED').catch(err => 
    ...
    const toggleCall = async () => {
        ...
        if (userPlan === 'ai_basic' && monthlyCallCount >= 10) {
            toast.error('월 10회 제한에 도달했습니다. AI Pro로 업그레이드하시면 무제한 통화가 가능합니다.');
            return;
        }
    ...
    // 사용량 표시
    const callLimitDisplay = userPlan === 'ai_basic' ? (
        <div className="text-center mb-4">
            <p className="text-sm text-muted-foreground">
                이번 달 사용량: <span className="font-semibold text-foreground">{monthlyCallCount}/10</span>
            </p>
        </div>
    ) : userPlan === 'ai_pro' ? (
        <div className="text-center mb-4">
            <p className="text-sm text-muted-foreground">
                이번 달 사용량: <span className="font-semibold text-foreground">{monthlyCallCount}</span> (무제한)
            </p>
        </div>
    ) : null;
        ...
        <div className="max-w-5xl mx-auto px-4 flex flex-col overflow-hidden pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <Card className='bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative'>
                    <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Hospital Assistant</p>
                    ...
                    <div className="space-y-3">
                        ...
                        {callEnded && (
                            <div className="message-item animate-in fade-in duration-300">
                                ...
                                <p className="text-foreground">통화가 종료되었습니다. Hospital AI를 이용해주셔서 감사합니다!</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="w-full flex justify-center gap-4">
                <Button
                    ...
                    onClick={toggleCall}
                    disabled={connecting || callEnded || (userPlan === 'ai_basic' && monthlyCallCount >= 10)}
                >
                    {connecting && (
